...
Namespace WindowsApplication1
	Partial Public Class Form1
		...
		Private Function GetValue(ByVal row As DataRowView, ByVal prevFieldName As String, ByVal nextFieldName As String) As Integer
			Dim value1 As Decimal = CDec(row(prevFieldName))
			Dim value2 As Decimal = CDec(row(nextFieldName))
			If value1 = 0 Then
				Return 0
			End If
			Return CInt(Fix((value2 - value1) * 100 / value1))
		End Function
		...
		Private Sub gridView1_CustomUnboundColumnData(ByVal sender As Object, ByVal e As DevExpress.XtraGrid.Views.Base.CustomColumnDataEventArgs) Handles gridView1.CustomUnboundColumnData
			If e.IsGetData Then
				Dim nextColumnFieldName As String = gridView1.VisibleColumns(e.Column.VisibleIndex + 1).FieldName
				Dim prevColumnFieldName As String = gridView1.VisibleColumns(e.Column.VisibleIndex - 1).FieldName
				e.Value = GetImageByValue(GetValue(CType(e.Row, DataRowView), prevColumnFieldName, nextColumnFieldName))
			End If
		End Sub
	End Class
End Namespace